...
<div class="meta">
  <span><strong>Date:</strong> 2026-01-08</span>
...
**Check pentester's inbox** using the `GET /api/mail` endpoint:
...
- **Internal exfiltration for stealth:** Using the application's own mail system (`/api/mail/send` → `GET /api/mail`) avoids external network calls that trigger firewall alerts
